/**
 * MCCARTHY 2026 REPLAY ENGINE — Pass 88 (2026-05-15)
 * Fetches Alec McCarthy's 2026 MLB game log, computes days-based rolling windows,
 * weekly DLR snapshots, an April settlement, and optionally refreshes
 * games/jake_mccarthy-2026.json.
 *
 * Source: MLB Stats API — player 664983, sport_id 1, season 2026.
 * Engine version: P60.
 *
 * Usage:
 *   node scripts/replay-bohm-2026.mjs
 *   node scripts/replay-bohm-2026.mjs --write-json
 */
...
const PLAYER_ID = "jake_mccarthy"
const MLBAM_ID = 664983
const SEASON = 2026
const ENGINE_VERSION = "P60"
const BASELINE_AVG = 0.204
const BASE_DLR = 41.7
...
async function fetchGames() {
  const url = `https://statsapi.mlb.com/api/v1/people/${MLBAM_ID}/stats?stats=gameLog&sportId=1&season=${SEASON}&group=hitting&gameType=R`
...
console.log("=== MCCARTHY 2026 REPLAY — PASS 78 COMPUTATION OUTPUT ===\n")
...
if (WRITE_JSON) {
  const out = {
    ...
    source_url: `https://statsapi.mlb.com/api/v1/people/${MLBAM_ID}/stats?stats=gameLog&sportId=1&season=${SEASON}&group=hitting&gameType=R`,
  ...
  writeFileSync(join(ROOT, "games", "jake_mccarthy-2026.json"), JSON.stringify(out, null, 2) + "\n")
  console.log("\nWrote games/jake_mccarthy-2026.json")
}
