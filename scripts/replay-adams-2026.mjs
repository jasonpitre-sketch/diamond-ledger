/**
 * ADAMS 2026 REPLAY ENGINE — Pass 79 (2026-05-15)
 * Fetches Brice Adams's 2026 MLB game log, computes days-based rolling windows,
 * weekly DLR snapshots, an April settlement, and optionally refreshes
 * games/jordyn_adams-2026.json.
 *
 * Source: MLB Stats API — player 677941, sport_id 11, season 2026.
 * Engine version: P60.
 *
 * Usage:
...
const PLAYER_ID = "jordyn_adams"
const MLBAM_ID = 677941
const SEASON = 2026
const ENGINE_VERSION = "P60"
const BASELINE_AVG = 0.213
const BASE_DLR = 45.0
const LARGE_MOVEMENT_THRESHOLD = 2.0

const TEAM_ABBR = { 247: "BIR", 4124: "PNS", 559: "RCT", 498: "CHA", 102: "RR", 416: "LOU", 568: "NOR", 451: "IOW", 494: "CLT", 445: "COL" }
...
async function fetchGames() {
  ...
  for (const sportId of [11, 12]) {
    const url = `https://statsapi.mlb.com/api/v1/people/${MLBAM_ID}/stats?stats=gameLog&sportId=${sportId}&season=${SEASON}&group=hitting&gameType=R`
    const data = await fetch(url).then(res => { if (!res.ok) throw new Error(`MLB Stats API HTTP ${res.status}`); return res.json() })
    splits = splits.concat(data?.stats?.[0]?.splits ?? [])
  }
...
console.log("=== ADAMS 2026 REPLAY — PASS 79 COMPUTATION OUTPUT ===\n")
...
if (WRITE_JSON) {
  ...
  writeFileSync(join(ROOT, "games", "jordyn_adams-2026.json"), JSON.stringify(out, null, 2) + "\n")
  console.log("\nWrote games/jordyn_adams-2026.json")
}
