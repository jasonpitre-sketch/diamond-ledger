/**
 * BREMNER 2026 REPLAY ENGINE — Pass 81 (2026-05-15)
 * Fetches Tyler Bremner's 2026 High-A start log, computes starts-based rolling
 * windows, weekly DLR snapshots, an April settlement, and optionally refreshes
 * games/tyler_bremner-2026.json.
 *
 * Source: MiLB Stats API — player 803285, sport_id 13, season 2026.
 * Engine version: P60.
 *
 * Usage:
 *   node scripts/replay-bremner-2026.mjs
 *   node scripts/replay-bremner-2026.mjs --write-json
 */
...
const PLAYER_ID = "tyler_bremner"
const MLBAM_ID = 803285
const SEASON = 2026
const ENGINE_VERSION = "P60"
const BASELINE_ERA = 3.49
const BASE_DLR = 63.0
...
async function fetchStarts() {
  const url = `https://statsapi.mlb.com/api/v1/people/${MLBAM_ID}/stats?stats=gameLog&sportId=13&season=${SEASON}&group=pitching&gameType=R`
  const data = await fetch(url).then(res => {
    if (!res.ok) throw new Error(`MiLB Stats API HTTP ${res.status}`)
    return res.json()
  })
  const splits = data?.stats?.[0]?.splits
  if (!Array.isArray(splits)) throw new Error("Unexpected MiLB Stats API response shape")
...
console.log("=== BREMNER 2026 REPLAY — PASS 81 COMPUTATION OUTPUT ===\n")
...
if (WRITE_JSON) {
  const out = {
    ...
    source: "MiLB Stats API gameLog",
    source_url: `https://statsapi.mlb.com/api/v1/people/${MLBAM_ID}/stats?stats=gameLog&sportId=13&season=${SEASON}&group=pitching&gameType=R`,
  ...
  writeFileSync(join(ROOT, "games", "tyler_bremner-2026.json"), JSON.stringify(out, null, 2) + "\n")
  console.log("\nWrote games/tyler_bremner-2026.json")
}
